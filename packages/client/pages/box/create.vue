...
const model = ref({
  ...
  max_supply: 100,
  mint: '5VTWJhA9YzmrFmauMd3JYop4TouP2oF5nSXb5KWkYRSh',
  master_edition: '',
  metadata: '',
  token_account: '',
})
    ...
    const created = await nftbox.value?.boxset()?.create(
      {
        ...
        max_supply: model.value.max_supply,
      },
      {
        masterEdition: new PublicKey(model.value.master_edition),
        metadata: new PublicKey(model.value.metadata),
        tokenAccount: new PublicKey(model.value.token_account),
      }
    )
